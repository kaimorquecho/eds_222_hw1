...
The population is the city of Lahore, Pakistan between early Nov 2018 and late Nov 2019.

2.  What are the *samples* in this setting? Please be precise.

There are two samples - One sample are the households in Lahore who have volunteered to install a monitor in their home between early Nov 2018 and late Nov 2019, and the other is the locations in Lahore that the government has chosen to place monitors at, between early Nov 2018 and late Nov 2019.
...
str(pooled_1000) # dataset dimensions 
...
str(strat_random_sample) # dataset dimensions
...
```{r}
# calculate the mean for stratified random sample and for the pooled n = 1000 sample since I had not calculated those 
...
The stratified random sample matches the satelite data best at 67.491 $\mu g/m^3$. This sample contains a more representative collection of observations because it pulls from both the govt data and the crowdsourced data, but using strata. Stratifying the sample and taking 200 obs from each strata ensures that the sample is representative of PM 2.5 concentrations at every latitude subgroup, a level of precision that cannot be achieved by simply taking a random  n = 1000 sample of the pooled govt and crowdsourced observations. The crowdsourced data will still be relied upon more than the govt data, because crowdsourced obs make up the majority of the obs in the pooled sample, but stratifying makes our sample more representative of the population despite this disproportion in obs. 

I would recommend to the Prime Minister that govt data be used to calculate the best estimate of $\mu g/m^3$, but with the condition that monitors be distributed all over Lahore, not only where convinient. 
