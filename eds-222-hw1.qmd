...
ggplot() +
  ...
  scale_color_manual(values = c("Crowdsourced" = "firebrick", "Govt" = "navyblue")) +
  xlab("Longitude(°)") + 
  ylab("Latitude(°)") +
  theme_bw() +
...
Yes, the locations of monitors in Q4 support the Dawn's claims. The distribution of the monitors for the govt sample are clustered together between 74.325 and 74.337° longitude and 31.57° and less than 31.58° latitude. This means the govt sampled a very small, specific area of Lahore, resulting in data that is purposefully under-reporting air pollution by being selective in monitor placement. The sample is not representative of the population (Lahore, Pakistan).
...
strat_sample <- pooled_samples %>% # stratifying into strata of 0.01 degrees of latitude. 
  ...
  group_by(lat_strata) # note for self: we don't order from min to max before stratifying because we want to take a random sample eventually
...
The crowdsourced sample matches the satelite data best at 70.2  $\mu g/m^3$. This makes sense given that the sample is the closest to a random, representative sample out of all the samples taken. 

However, even though it is the closest, it is still biased as discussed earlier. I would recommend to the Prime Minister that govt data be used to calculate the best estimate of $\mu g/m^3$, but with the condition that monitors be distributed all over Lahore, not only where convinient. While it would be good to gather crowdsourced data, there will always be a concern for self-selection bias, unless there be a significant financial investment to subsidize any cost for all homes to have a monitor or to create the socio-economic and geographic conditions in Lahore so that crowdsourced monitoring can be representative of the entire population. 

There are also issues in relying on govt data due to the possibility of corruption, but there are strategies that can be implemented to mitigate this risk at a lower cost than the alternative. I would recommend that a civilian task-force be created to supervise the placement of monitors by govt officials. I'd recommend that the distribution and amount of monitors be proportional to the population size of each district in Lahore.
